...
//  if(!sd.begin(SD_SEL, SPI_HAVE_SPEED)) sd.initErrorHalt();
...
// Below is only needed if not interrupt driven. Safe to remove if not using.
  ...
  //-------------------------------------------------------------------------
      ...
      if (fn_index>9999)
      {
        ...
        dirbuffer2[0]='/';Serial.println(dirbuffer2);sd.chdir((dirbuffer2),true);}else
      {
      SdFile file;
      char filename[60];
       
      sd.chdir(dirbuffer2,true);              //ak 10000 tak root directory
...
//      sd.ls(LS_R | LS_DATE | LS_SIZE);
    //sd.ls("/"); //korenovy adresar
          sd.ls(dirbuffer2,true); 
